...
class AbsenceCalendarUserActionListener implements IParameterizedEventListener
{
    ...
    public function execute($eventObj, $className, $eventName, array &$parameters)
    {
        ...
        if (!isset($params['data']['absentFrom']) || !isset($params['data']['absentAuto'])) {
            return;
        }

        if (ABSENCE_CALENDAR_HIDEAUTO) {
            if ($params['data']['absentAuto']) {
                return;
            }
        }
        ...
        foreach ($objects as $userEditor) {
            ...
            // need permission
            if (!ABSENCE_CALENDAR_FORCE && !WCF::getSession()->getPermission('user.calendar.canCreateEvent')) {
                return;
            }
            ...
            if (ABSENCE_CALENDAR_CHOOSE) {
                if (isset($params['data']['absentCalendar'])) {
                    if (!$params['data']['absentCalendar']) {
                        return;
                    }
                } else {
                    if (isset($user->absentCalendar) && !$user->absentCalendar) {
                        return;
                    }
                }
            }
